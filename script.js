...
function onResize() {
  ...
  smokeCanvas.style.setProperty('height', `${s3Ground.getBoundingClientRect().top - svgRect.top
    }px`)
...
function step(now) {
  ...
  if (s5 >= 0 && s5 <= 1) {
    ...
    s5Spacecraft.style.setProperty('transform', `rotate(${-115 +
      cubicBezier(.2, 0, .5, 1, (p - 0.175) / 0.35) * 195 +
      cubicBezier(.2, 0, .8, 1, (p - 0.65) / 0.275) * -80
      }deg)`)

    s5SpacecraftFire.style.setProperty('transform', `scale(${cubicBezier(.1, 0, .8, 1, (p - 0.425) / 0.03) -
...
function spawn(x, y) {
  ...
  particle.init(x, y, random(10, 15))
